package de.codeelements.bhfliveutil.provider
...
//        "de.codeelements.bhfliveutil.provider" // same as manifest -> provider android:authorities
...
//    }
            ...
            cursor?.let {
                ...
                if (it.moveToFirst()) {
                    if (!it.isAfterLast) {
                        ...
                        if(key.isNotEmpty() && key[0]!='#') // selfChange ?
    ...
    private fun getUrl(tableName:String) : String {
    ...
    fun registerValueChangeObserver(  context:Context, changedValueHandler : (key:String, value:String)->Unit) : ContentObserver? {
        ...
        var dataChangeObserver = DataChangeObserver(context, changedValueHandler)
