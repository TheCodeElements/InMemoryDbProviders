...
{
   val field_name_values_id = "id"
   val field_name_values_name = "var_name"
   val field_name_values_type = "var_type"
   val field_name_values_value = "var_value"
...
class ContentProviderTableDebug(db:SQLiteDatabase, providerName:String, tableName:String) :
    ContentProviderTable(db, providerName, tableName.lowercase())
{
 val field_name_debug_receive_date_time = "receive_date_time"
 val field_name_debug_key = "debug_key"
 val field_name_debug_value = "debug_value"
    ...
    override fun createQuery(queryBuilder : SQLiteQueryBuilder, sortOrder:String?) : String {
        ...
        queryBuilder.projectionMap = values
        if (sortOrder == null || sortOrder === "") {
            return field_name_debug_receive_date_time
        }
        return sortOrder
    }
...
//        }
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri {
        ...
        if (rowID > 0) {
            val uriWithRowId = ContentUris.withAppendedId(getUri(tables[code - 1].tableName), rowID)
    ...
    companion object {
        ...
        fun initMe(db: SQLiteDatabase, provider: InMemoryDbProvider) {
            provider.addTable(ContentProviderTableValues(db, PROVIDER_NAME, TABLE_NAME_VALUES))
            provider.addTable(ContentProviderTableDebug(db, PROVIDER_NAME, TABLE_NAME_DEBUG))
        }
